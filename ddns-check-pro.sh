...
## 建立新的中转规则
# ========== 建立新的中转规则（关键修改：添加-s参数限制源IP）==========
echo "$(date) - 建立新的中转规则，仅允许来源IP: $allowed_source"
iptables -t nat -A PREROUTING -s $allowed_source -p tcp --dport $localport -j DNAT --to-destination $remote:$remoteport
iptables -t nat -A PREROUTING -s $allowed_source -p udp --dport $localport -j DNAT --to-destination $remote:$remoteport
